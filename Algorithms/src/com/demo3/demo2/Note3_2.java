...
public class Note3_2 {
    ...
    3.2.1 基本实现
        ...
        3.2.1.4 递归
            ...
            新结点会连接到树底层的空链接上，树的其他部分则不会改变。例如：第一个被插入的键就是根结点，第二个被插入的就是根节点的两个子结点之一，以此类推。因为每个结点都含有两个链接，树会逐渐长大而不是
            萎缩。不仅如此，因为只有查找或者插入路径上的结点才会被访问，所以随着树的增长，被访问的结点数量占数的总结点数的比例也会不断地降低。
     3.2.2 分析
            使用二叉查找树的算法的运行时间取决于树的形状，而树的形状又取决于键被插入的先后顺序。在最好的情况下，一棵含有N个结点的树是完全平衡的，每条空链接和根节点的距离都为~lgN.在最坏的情况下，搜索路径上
            有可能有N个结点，但在一般情况下树的形状和最好情况更接近。
     3.2.3 有序性相关的方法与删除操作
            二叉查找树得以广泛应用的一个重要原因就是它能够保持键的有序性，因为它可以作为实现有序符号表API中的众多方法的基础。这使得符号表的用例不仅能够通过键还能通过键的相对顺序来访问键值对。
        3.2.3.1 最大值和最小值
            如果根节点的左链接为空，那么一棵二叉树最小的键就是根节点；如果左链接非空，那么树中的最小键就是左子树中的最小键。这不仅描述了类【BST】中min()方法的递归实现。同时也递推地证明了它能在二叉查找树
            中找到最小的键。
            见类【BST】min()方法
        3.2.3.1 向上取整和向下取整
            如果给定的键key小于二叉查找树的根结点的键，那么小于等于key的最大键floor(key)一定在根结点的左子树中；如果给定的键大于二叉查找树的根结点，那么只有当根节点右子树中存在小于等于key的结点时，小于
            等于key的最大键才会出现在右子树中。这段描述说明了floor()方法的递归实现，同时也递推的证明了它能够计算出预期的结果。将“左”变为“右”就能得到ceiling()的算法。
